...
`endif
  ...
  `endif
    ...
    // Connect dcache <-> MVU (read path)
    mkConnection(dcache.get_read_mem_req, mvu.put_cache_read_req);
    mkConnection(mvu.get_cache_read_resp, dcache.put_read_mem_resp);

    // Connect MVU <-> TreeMemory (tree node read/write)
    mkConnection(mvu.get_tree_node_req, tree_mem.put_req);
    mkConnection(tree_mem.get_resp, mvu.put_tree_node_resp);
    mkConnection(mvu.get_tree_write_req, tree_mem.put_write_req);
    mkConnection(tree_mem.get_write_resp, mvu.put_tree_write_resp);
