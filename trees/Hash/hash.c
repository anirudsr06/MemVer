...
#define MTIME_ADDR 0X200BFF8UL
volatile uint64_t *mtime_ptr = (volatile uint64_t *)MTIME_ADDR;
...
void main (){
  ...
  char hash[65] = {0};
  int iter = 10000;
  uint64_t start = *mtime_ptr;
  for (int i=0; i<iter; i++){
    sha256_easy_hash_hex(msg, strlen(msg), hash);
  }
  uint64_t end = *mtime_ptr;
  printf("Hash time taken: %lu - for %d iterations\n", end-start, iter);
  return;
}
