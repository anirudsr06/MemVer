...
`endif
  ...
  interface Ifc_ccore_axi4;
		...
		interface AXI4_Master_IFC#(`paddr, ELEN, USERSPACE) master_t;
...
`endif
		...
		AXI4_Master_Xactor_IFC #(`paddr, ELEN, USERSPACE) tree_xactor <- mkAXI4_Master_Xactor;
  `ifdef dcache
...
rg_shift_amount:%d",hartid, req.data, rg_burst_count, last, rg_shift_amount))
  ...
  `endif
    ...
    // ========================= Tree Memory AXI4 Bridge =========================
    // Tree Read Request: DCache_mem_readreq -> AXI4 Read Address
    rule rl_handle_tree_read_request;
      let req <- dmem.get_tree_read_mem_req.get;
      AXI4_Rd_Addr#(`paddr, 0) tree_rd_req = AXI4_Rd_Addr {
        araddr  : truncate(req.address),
        aruser  : ?,
        arlen   : req.burst_len,
        arsize  : req.burst_size,
        arburst : 'b01,  // INCR burst for sequential tree nodes
        arid    : 0,
        arprot  : {1'b0, 1'b0, curr_priv[1]}
      };
      tree_xactor.i_rd_addr.enq(tree_rd_req);
      `logLevel( core, 1, $format("[%2d]CORE : Tree Read Request ",hartid, fshow(tree_rd_req)))
    endrule

    // Tree Read Response: AXI4 Read Data -> DCache_mem_readresp
    rule rl_handle_tree_read_resp;
      let fab_resp <- pop_o(tree_xactor.o_rd_data);
      Bool bus_error = !(fab_resp.rresp == AXI4_OKAY);
      dmem.put_tree_read_mem_resp.put(DCache_mem_readresp {
        data : truncate(fab_resp.rdata),
        last : fab_resp.rlast,
        err  : bus_error
      });
      `logLevel( core, 1, $format("[%2d]CORE : Tree Read Response ",hartid, fshow(fab_resp)))
    endrule
    ...
    rule rl_handle_tree_write_resp;
      let response <- pop_o(tree_xactor.o_wr_resp);
      let bus_error = !(response.bresp == AXI4_OKAY);
      dmem.put_tree_write_mem_resp.put(!bus_error);
      `logLevel( core, 1, $format("[%2d]CORE : Tree Write Response ",hartid, fshow(response)))
    endrule
    // ===========================================================================
...
`endif
  ...
  `endif
    ...
    endinterface;
		...
		interface master_t = tree_xactor.axi_side;
    `ifdef rtldump
